...
public class ViewAlbum extends HttpServlet {
	...
	private void displayAlbum(PhotoAlbum pa, String label, PrintWriter out) {
		...
		for (int j = 0; j < pa.getPhotoCount(); j++) {
			out.println("<td>");
			out.println("<a href='./DisplayPhoto?photo=" + j + "'>");
			out.println("<img src='./DisplayPhoto?photo=" + j + "' alt='photo' height='120' width='150'> ");
			out.println("</a>");
			out.println("</td>");
		}
		out.println("<td bgcolor='#cccccc' width='120' height='120'>");
		out.println("<form align='left' action='ViewAlbum' method='post' enctype='multipart/form-data'>");
		...
		for (int j = 0; j < pa.getPhotoCount(); j++) {
			...
			out.println("<a href='RemovePhoto?photo=" + j + "'>remove</a>");
			out.println("</td>");
		}
		out.println("</tr>");
		out.println("</table>");
